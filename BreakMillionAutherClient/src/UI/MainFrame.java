 ...
 //   static MediaPlayer mp,mp2;
        ...
        mainframe.addMouseListener(new MouseAdapter(){
			public void mouseClicked(MouseEvent event){
		       // if (jindu<1080) jindu=1080;
		        ...
		        if (login(zt.getText(),new String(zp.getPassword()))){
	...
	public static boolean login(String name,String password){
       ...
       timer2.schedule(new MyTask2(),40, 40);
        boolean flag=false;
		PlayerBLService player=rmi.RemoteFactory.getPlayerBLService();
		String m =null;
		try {
			System.out.println(name+"  "+password);
			m = player.login(name,password);
		...
		if(m.equals("密码不正确")){
			zp.setText("");
			zp.requestFocusInWindow();
			System.out.println("密码不正确");
			
		}
		else if(m.equals("用户名不存在")){
			zt.setText("");
			zt.requestFocusInWindow();
